            ...
            if (juzhenType == 4) {//数字矩阵
                ...
                if (rowSelect < all) {
                    ...
                    if (currentQuestionId == questionItemId) {
                        ...
                        if (min != 0 || max != 0) {
                            errorInfo = "请完成所有的输入项,每个大题总分为10,每个选项最小值" + min + "最大值" + max;
                        }
...
function confirmForm() {
    ...
    $.post("/Wenjuan/Submit", $("#form1").serialize(), function (data) {
        if (data.indexOf("ok:") != -1) {
            ...
            var answerPwd = data.replace("ok:", "");
            window.location.href = "/Wenjuan/GoingResult/" + code + "?answerPwd=" + answerPwd;
        }
        else {
...
passiveConfirmForm();
function passiveConfirmForm() {
    var questionnaireTypeHidden = $("#questionnaireTypeHidden").val();
    var totalExamTimeSecond = $("#totalExamTimeSecond").val();
    if (questionnaireTypeHidden == 2 && totalExamTimeSecond>0) {//考试问卷，并且设置了考试时间
        $("#lastSecondDiv").text(totalExamTimeSecond);
        var t = setInterval(function () {
            totalExamTimeSecond--;
            $("#lastSecondDiv").text(totalExamTimeSecond);
            if (totalExamTimeSecond == 0) {
                clearInterval(t);
                $("#initiativeConfirm").val(0);
                confirmForm();
            }
        }, 1000);
    }
